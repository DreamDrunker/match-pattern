 ...
 * @returns {any}
 */
export function compile_match_plan(program) {
    const ret = wasm.compile_match_plan(program);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function match_pattern(data, patterns) {
    const ret = wasm.match_pattern(data, patterns);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('match_pattern_rs_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
